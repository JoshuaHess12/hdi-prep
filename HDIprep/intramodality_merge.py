...
#Run for full data
im1 = hdi_reader.HDIreader(path_to_data = "D:/Josh_Hess/01_10_19_MSI_peak_pick_20191025/Methods_Analysis_20190110/DFU/imc/DFU_ROI_04/DFU_ROI_04.ome.tiff",\
    path_to_markers=None,flatten=True,subsample=None)
#Run for subsampled data
im2 = hdi_reader.HDIreader(path_to_data = "D:/Josh_Hess/01_10_19_MSI_peak_pick_20191025/Methods_Analysis_20190110/DFU/imc/DFU_ROI_24/DFU_ROI_24.ome.tiff",\
    path_to_markers=None,flatten=True,subsample=None)
...
#Run for subsampled data
im3 = hdi_reader.HDIreader(path_to_data = "D:/Josh_Hess/01_10_19_MSI_peak_pick_20191025/Methods_Analysis_20190110/DFU/imc/DFU_ROI_04/DFU_ROI_04.ome.tiff",\
    path_to_markers=None,flatten=True,method='random',n=0.9,subsample=True)
#Run for subsampled data
im4 = hdi_reader.HDIreader(path_to_data = "D:/Josh_Hess/01_10_19_MSI_peak_pick_20191025/Methods_Analysis_20190110/DFU/imc/DFU_ROI_24/DFU_ROI_24.ome.tiff",\
    path_to_markers=None,flatten=True,method='random',n=0.9,subsample=True)
#Concatenate the data
...
#Get the sample name
key = list(test1.set_dict.keys())[1]

#Get the distances between objects -- assuming pixel order is the same
from scipy.spatial.distance import pdist
og_dist = pdist(test1.UMAP_embeddings[key].iloc[:5500])
embed_dist = pdist(test2.UMAP_embeddings[key].iloc[:5500])
...
#Now check for equality of local neighborhoods
import sklearn
knn_dist1 = sklearn.neighbors.kneighbors_graph(test1.UMAP_embeddings[key], \
     n_neighbors=105, mode='distance', metric='minkowski', p=2, metric_params=None, include_self=False, n_jobs=None)
booleanarr1 = knn_dist1.astype(np.bool)

knn_dist2 = sklearn.neighbors.kneighbors_graph(test2.UMAP_embeddings[key], \
     n_neighbors=105, mode='distance', metric='minkowski', p=2, metric_params=None, include_self=False, n_jobs=None)
...
for i in range(booleanarr2.shape[1]):
    ...
    #jacc.append(scipy.spatial.distance.jaccard(booleanarr1[i][tmp], booleanarr2[i][tmp]))
    jacc6.append(scipy.spatial.distance.jaccard(booleanarr1[i][tmp], booleanarr2[i][tmp]))


np.array(jacc).mean()
import seaborn as sns
dat = pd.concat([pd.DataFrame({"Jaccard Distance":jacc6,"NN":"96"}),pd.DataFrame({"Jaccard Distance":jacc5,"NN":"95"}),pd.DataFrame({"Jaccard Distance":jacc3,"NN":"75"}),pd.DataFrame({"Jaccard Distance":jacc,"NN":"55"}),\
    pd.DataFrame({"Jaccard Distance":jacc4,"NN":"35"}),pd.DataFrame({"Jaccard Distance":jacc2,"NN":"15"})],axis=0)

sns.violinplot(y = "Jaccard Distance",x="NN",data = dat,hue = "NN",dodge=False)
